...
//	bls_l2cap_requestConnParamUpdate (8, 8, 99, 400);  // 1S long sleep
	...
	bls_l2cap_requestConnParamUpdate (24, 24, 0, 400);  // 1S long sleep

	device_in_connection_state = 1;
	device_connection_tick = clock_time() | 1;
}

_attribute_data_retention_ int AA_disconnect = 0;
...
_attribute_ram_code_ void function_slave_coded_phy_init_deepRetn(void)
{
#if (FEATURE_DEEPSLEEP_RETENTION_ENABLE)
...
#else
	if(phy_update_test_tick && clock_time_exceed(phy_update_test_tick, 1000000)){
		phy_update_test_tick = clock_time() | 1;
