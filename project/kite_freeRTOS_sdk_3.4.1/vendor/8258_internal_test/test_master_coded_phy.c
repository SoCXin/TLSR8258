...
#include "vendor/common/blt_common.h"
	...
	#define			SPP_HANDLE_DATA_S2C			SPP_SERVER_TO_CLIENT_DP_H
	#define			SPP_HANDLE_DATA_C2S			SPP_CLIENT_TO_SERVER_DP_H

	int myC2SWrite(void * p){

	}

#if (0) // support RF RX/TX MAX data Length: 251byte
...
static int device_in_connection_state;
			...
			if(attHandle == SPP_HANDLE_DATA_S2C)
			{
				...
				if(len > 0)
				{
					...
					if(seq_num == seq_num_next){
						...
						if(BLE_SUCCESS == blc_gatt_pushWriteComand(BLM_CONN_HANDLE,  SPP_CLIENT_TO_SERVER_DP_H, app_test_data, 20)){
							app_test_data[0] ++;
						}
...
//	blt_conn_phy.conn_cur_CI = CODED_PHY_PREFER_S8;

}



_attribute_ram_code_ void function_master_coded_phy_init_deepRetn(void)
{
